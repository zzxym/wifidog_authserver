# WiFiDog AuthServer v2.0 - 双模式认证版

> 支持 **AD域LDAP认证** 与 **本地用户认证(SQLite)** 灵活切换的 WiFiDog 认证服务器
>
...
| 新特性 | 说明 |
|--------|------|
| **本地用户认证** | 新增 SQLite 本地用户系统，无需 AD 域控即可独立运行 |
| **双模式切换** | `.env` 中 `AUTH_MODE=ad` 或 `AUTH_MODE=local` 一键切换 |
| **本地用户管理** | 管理后台完整支持：新建、编辑、删除、启用/禁用本地用户 |
| **设备强制踢出** | 管理后台可一键踢出在线设备，token 自动加入黑名单 |
| **管理员会话** | Session 登录替代纯 token 方式，支持登出操作 |
...
| 功能 | 说明 |
|------|------|
| **双模式认证** | AD域LDAP认证 或 本地SQLite用户认证，灵活切换 |
| **本地用户管理** | 管理后台新建/编辑/删除/启用/禁用用户（local模式） |
...
```ini
# ===== 认证模式（v2.0新增）=====
# "ad" = AD域LDAP认证, "local" = 本地用户认证(SQLite)
AUTH_MODE=ad
...
```
============================================================
  WiFiDog AuthServer 启动 (双模式认证 + 自动清理 + 管理UI)
  认证模式   : 本地用户(SQLite)
...
```
┌─────────────────────────────────────────────────────┐
│  🔧 WiFiDog 管理后台    设备用户 → 本地用户 → 退出 │
├─────────────────────────────────────────────────────┤
│  [ 15 ]      [ 8 ]       [ 3 ]      [ 3 ]    │
│  在线设备总数  活跃用户数   僵尸设备数  默认最大设备 │
├─────────────────────────────────────────────────────┤
│  ⚙️ 全局设置                                      │
│  认证模式      : 本地用户(SQLite) / AD域(LDAP)      │
│  定时清理任务  : ✅ 已启用                         │
│  清理周期(Cron): 0 0 * * *                      │
│  全局默认心跳  : 168 小时                        │
│  [立即清理僵尸设备]  [设备用户管理]  [本地用户管理] │
└─────────────────────────────────────────────────────┘
```

### 设备用户管理页面（`/admin/users`）
...
├─────────────────────────────────────────────────────┤
│  📱 在线设备（1 台）       操作                    │
│  192.168.1.100  rg-ac  06-06 08:00  [踢出]        │
└─────────────────────────────────────────────────────┘
```

### 本地用户管理页面（`/admin/local_users`，local模式）
...
├─────────────────────────────────────────────────────┤
│  [搜索...]                           [搜索] [清除]  │
│  用户名   显示名称  邮箱    状态  角色    操作       │
│  testuser 测试用户  t@t.com 启用  管理员 [编辑][禁用][删除]│
│  guest    访客用户  -       启用  普通   [编辑][禁用][删除]│
└─────────────────────────────────────────────────────┘
```
...
│  ← 本地用户管理                  退出登录           │
├─────────────────────────────────────────────────────┤
│  创建本地用户 / 编辑用户: testuser                   │
│  用户名: [testuser        ]  (编辑时不可修改)        │
│  密码  : [                ]  留空不修改密码           │
│  显示名: [测试用户         ]                         │
│  邮箱  : [test@test.com    ]                         │
│  ☑ 启用账号    ☑ 管理员                             │
...
GET  /admin/logout             # 注销session
```

### 本地用户管理 API（local 模式）

```
GET  /admin/api/local_users?admin_token=xxx&page=1&search=keyword
                               # 获取本地用户列表（分页+搜索）
GET  /admin/local_users/create # 创建用户表单页
POST /admin/local_users/create # 提交创建用户
GET  /admin/local_users/<username>/edit     # 编辑用户表单页
POST /admin/local_users/<username>/edit     # 提交修改
POST /admin/local_users/<username>/toggle   # 切换启用/禁用
POST /admin/local_users/<username>/delete   # 删除用户
```
...
两种认证模式的流程完全一致，区别仅在于用户凭据的验证方式：

```
用户打开WiFi → 连接SSID → 打开浏览器访问任意网址
    → 锐捷AC重定向到 AuthServer /login 页面
    → 用户输入用户名 + 密码
    → AuthServer 根据 AUTH_MODE 配置进行认证:
         ad 模式 → 通过LDAP验证AD域控
         local模式 → 通过SQLite验证本地用户
...
### 本地模式（AUTH_MODE=local）

使用内置 SQLite 数据库存储用户信息，密码通过 bcrypt 安全哈希。适用于以下场景：
- 无 AD 域控环境的小型部署
- 独立的访客 WiFi 认证
- 测试和开发环境

**管理方式**：通过管理后台 → 本地用户管理，进行用户的增删改查和启用/禁用操作。
...
```
wifidog_authserver/
├── app.py              # 主应用，WiFiDog协议 + 双模式认证 + 管理员UI + 定时清理
├── ad_auth.py          # AD域LDAP认证模块（ldap3，纯Python）
├── local_auth.py       # 本地用户认证模块（SQLite + bcrypt）v2.0新增
...
| 问题 | 排查方法 |
|------|---------|
| 用户无法认证 | ad模式: 检查AD_SERVER是否可达；local模式: 检查用户是否启用 |
| 本地用户管理不可见 | 确认 `AUTH_MODE=local`，管理后台仅在local模式显示本地用户入口 |
